...
let wlimg = new Image();
wlimg.src = 'wavelet.png';
let imgloaded = false;


function clickHud(event){
	if(!imgloaded){
		imageContext.drawImage(wlimg, 0, 0);
	}
...
function getSoundFromImage(){
	...
	for (var row = 0; row<heightInPixels; row++){
		...
		pixelData = imageContext.getImageData(column*pixelWidth, row*pixelHeight, 1, 1).data[0];

		if(column<(widthInPixels-1)){
			nextPixelData = imageContext.getImageData((column+1)*pixelWidth, row*pixelHeight, 1, 1).data[0];
		} else {
			nextPixelData = 0;
		}
