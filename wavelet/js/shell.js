	...
	animate:function(){
		...
		for (var i=0; i<this.amount; i++){
			...
			if(controls.objects[i]==controls.selection){
				...
				if (this.poss && i==0) {
					controls.objects[i].position.x = (controls.objects[this.possby].position.x+(this.hook*Math.cos(controls.objects[this.possby].rotation.y - 1.5708)));
			...
			if(i>0){
				...
				if(this.anglediff[i]>3.1415927){this.anglediff[i] -= 6.24318}
				if(this.anglediff[i]<-3.1415927){this.anglediff[i] += 6.24318}


				if (Math.abs(this.anglediff[i])<.5) {
				...
				if(this.possby==i && this.targetd[i]<1){
						this.poss = false;
						this.possby = 0;
						this.yv[0] = this.yv[i] + .4*Math.sin(controls.objects[i].rotation.y - 1.5708);
						this.xv[0] = this.xv[i] + .4*Math.cos(controls.objects[i].rotation.y - 1.5708);
				...
				if(!this.poss){
					...
					if(Math.sqrt((Math.abs(xd)*Math.abs(xd)+(Math.abs(yd)*Math.abs(yd))))<.2){
						this.poss = true;
						this.possby = i;
					}
				}
				...
				if(this.poss){
					var xd = ball.mesh.position.x - (controls.objects[this.possby].position.x+(this.hook*Math.cos(controls.objects[this.possby].rotation.y - 1.5708)));
					var yd = ball.mesh.position.y - (controls.objects[this.possby].position.y+(this.hook*Math.sin(controls.objects[this.possby].rotation.y - 1.5708)));
					if(Math.sqrt((Math.abs(xd)*Math.abs(xd)+(Math.abs(yd)*Math.abs(yd))))>.3){
		...
		this.nearestB = 6;
		for (var i=1; i<6; i++){
			if (this.balld[i]<this.balld[this.nearestA]){
				this.nearestA = i;
			}
		}

		for (var i=6; i<this.amount; i++){
			if (this.balld[i]<this.balld[this.nearestB]){
				this.nearestB = i;
			}
		}
	}
...
var AI = {
	...
	animate:function(){
		...
		physics.targetx[physics.nearestA] = physics.targetx[physics.nearestB] = controls.objects[0].position.x;
		physics.targety[physics.nearestA] = physics.targety[physics.nearestB] = controls.objects[0].position.y;
...
var viewport = {
	...
	init:function(){
		...
		this.renderer = new THREE.WebGLRenderer( { clearColor: 0x111111 , antialias: false } );
	...
	animate:function() {
    	...
    	//lightning.animate();
    	lights.light4.position.set( ball.mesh.position.x, ball.mesh.position.y, .5);
    	physics.animate();
    	AI.animate();
...
var controls = {
	...
	onDocumentMouseDown:function(event){
    	...
    	var vector = new THREE.Vector3(mouseX, mouseY, 1); // Get 3D vector from 3D mouse position using 'unproject' function
  		...
  		var intersects = controls.raycaster.intersectObjects(controls.objects);
  		if (intersects.length > 0) {
    			...
    			var intersects = controls.raycaster.intersectObject(controls.plane, true);
    			controls.offset.copy(intersects[0].point).sub(controls.plane.position);
  		}
	},
	onDocumentMouseMove:function(event) {
		//log.out("controls mm");
  		...
  		if (controls.selection) {
    			...
    			var intersects = controls.raycaster.intersectObject(controls.plane);
    			// Reposition the object based on the intersection point with the plane
    			var oldZ = controls.selection.position.z;
    			controls.selection.position.copy(intersects[0].point.sub(controls.offset));
    			controls.selection.position.z = oldZ;
  		} else {
    			// Update position of the plane if need
    			var intersects = controls.raycaster.intersectObjects(controls.objects);
    			if (intersects.length > 0) {
      				controls.plane.position.copy(intersects[0].object.position);
...
var resizer = {
	...
	init:function(){
		...
		this.tanFOV = Math.tan( ( ( Math.PI / 180 ) * viewport.camera.fov / 2 ) );
		this.windowHeight = window.innerHeight;
	},
	onWindowResize:function( event ) {
    		viewport.camera.aspect = window.innerWidth / window.innerHeight;
    		viewport.camera.fov = ( 360 / Math.PI ) * Math.atan( resizer.tanFOV * ( window.innerHeight / resizer.windowHeight ) );
...
var lights = {
	...
	init:function(){
		log.out("init lights");
		this.ambientLight = new THREE.AmbientLight(0xAAAAAA, .6);

		this.light1 = new THREE.PointLight( 0xff0000, 2, 20, 2);
		this.light1.position.set( 13, 0, 2 );
		

		
		this.light2 = new THREE.PointLight( 0x00ff00, 2,  20, 2);
		...
		this.light3 = new THREE.PointLight( 0xeeeeff, .5,  100, 1 );
		...
		//this.light5 = new THREE.PointLight( 0x3333ff, .5,  30, 2 );
		//this.light5.position.set( 2, 2, 0 );

		//this.light6 = new THREE.PointLight( 0x0000ff, .5, 40, 2 );
		//this.light6.position.set( 0, 5, 3.1 );
		

		
		//this.light7 = new THREE.PointLight( 0xffff00, .5,  40, 2 );
		//this.light7.position.set( 0, -5, 3.1 );
		
		//this.light8 = new THREE.PointLight( 0x9999ff, .5,  30, 2 );
...
var field = {
	fieldSkin:null,
	fieldSkinTexture:null, fieldMap:null, fieldMapTexture:null, ctx:null,
	init:function(){
		log.out("init field");
		...
		ctx.fillStyle = 'black';
		ctx.beginPath();
		ctx.moveTo(0, fieldMap.height*.125);
	    	ctx.lineTo(fieldMap.width, fieldMap.height*.125);
	    	ctx.moveTo(0, fieldMap.height*.625);
	    	ctx.lineTo(fieldMap.width, fieldMap.height*.625);
	    	ctx.moveTo(0, fieldMap.height*.375);
	    	ctx.lineTo(fieldMap.width, fieldMap.height*.375);
	    	...
	    	ctx.moveTo(0, fieldMap.height*.75);
	    	ctx.lineTo(fieldMap.width, fieldMap.height*.75);
	    	ctx.moveTo(0, fieldMap.height*.865);
	    	ctx.lineTo(fieldMap.width, fieldMap.height*.865);
	    	ctx.stroke();
		...
		fieldMaterial = new THREE.MeshPhongMaterial({ map:fieldSkinTexture, side:THREE.DoubleSide});
		fieldMaterial.bumpMap = fieldMapTexture;
		fieldMaterial.bumpScale = .01;
		...
		borderMtrlBack = new THREE.MeshPhongMaterial({color:0x444444});
		...
		borderMtrlFront = new THREE.MeshPhongMaterial({color:0x444444});
		...
		borderMtrlLeft = new THREE.MeshBasicMaterial({color:0x33cc33});
		...
		borderMtrlRight = new THREE.MeshBasicMaterial({color:0xcc3333});
		...
		goalMtrlLeft = new THREE.MeshPhongMaterial({color:0x444444});
		...
		goalMtrlRight = new THREE.MeshPhongMaterial({color:0x444444});
...
var ball = {
	...
	init:function(){
		...
		this.material = new THREE.MeshBasicMaterial({ color:0xffffff});//, transparent:true, opacity:.5});
		this.mesh = new THREE.Mesh( this.geometry, this.material );
		viewport.scene.add(this.mesh);
		this.mesh.position.set( 2, 2, .15 );
...
var lightning = {
    ...
    if(physics.poss){
      ...
      this.ax = ball.mesh.position.x + .22; 
      this.ay = ball.mesh.position.y;
      this.az = ball.mesh.position.z;
      this.bx = ball.mesh.position.x - .22;
    ...
    for (var p = 0; p < this.amount; p++) {
      var pX = this.ax + (((this.bx - this.ax)/this.amount)*p);// + (Math.sin(p)*.05);
          ...
          pZ = this.az + (((this.bz - this.az)/this.amount)*p) + (Math.sin(p/10)*.05)+ (Math.sin(p)*.01)+ (Math.sin((p/this.amount)*3.141)*.4);
         
          this.geometry.vertices[p].x = pX + ((Math.random()*.05)-.025);
          this.geometry.vertices[p].y = pY + ((Math.random()*.05)-.025);
          this.geometry.vertices[p].z = pZ + ((Math.random()*.05)-.025);
...
var car = {
	...
	init:function(){
		...
		this.material3 = new THREE.MeshStandardMaterial({ map:this.texture3 , shininess:90});
		this.material4 = new THREE.MeshStandardMaterial({ map:this.texture4 , side:THREE.DoubleSide, shininess:90});

		this.geometry1 = new THREE.SphereGeometry(.5,16,16, 0, Math.PI*2, 0, Math.PI/2);
